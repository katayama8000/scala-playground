...
graph TD
    ...
    subgraph "sendMessage の内部処理"
        ...
        D --> E(2. DH鍵合意: 新しい秘密鍵 + 相手の公開鍵 -> Shared Secret);
        E --> F(3. ルート鍵の更新 KDF: 古い rootKey と Shared Secret から 新しい rootKey と receivingChainKey を導出);
        F --> G(4. 送信チェーン鍵の更新 KDF: 古い sendingChainKey から 新しい sendingChainKey を導出);
        G --> H(5. メッセージの暗号化: 更新前の sendingChainKey を使って暗号化);
        H --> I(6. 新しいセッション状態を返す: 新しい鍵を持つ SignalSession と結果を返す);
    end
